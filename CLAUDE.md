...
- Unit tests: **≥ 509** (поточна планка після `fix-startlink-wire-shape` landing RG10 `WireShapeAnnotationTests` +1 + `DeviceLinkingSerializationTests` +2).
...
| RG10 | `RegressionGuards/WireShapeAnnotationTests.cs` | Кожна публічна властивість кожного DTO, зареєстрованого у `SignalJsonContext`, несе явний `[JsonPropertyName]` (або `[JsonIgnore]`). Контекст case-sensitive і без `PropertyNamingPolicy` — PascalCase без атрибута тихо десеріалізується у `null` на camelCase wire (баг `StartLinkResponse.DeviceLinkUri`, 2026-07-16). Wrapper-record'и з власним `[JsonConverter]` звільнені |
...
**Pending changes:** `fix-startlink-wire-shape` (**4.10.1**) — silent-null wire-bug fix у `StartLinkResponse` (відсутній `[JsonPropertyName("deviceLinkUri")]`) + новий RG10 (`WireShapeAnnotationTests`). Archive post-merge.
