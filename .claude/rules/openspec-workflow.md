...
- **README + `docs/api/` PR-time triggers.** Re-check README **AND** every potentially-affected `docs/api/*.md` (+ examples under `docs/examples/`) when ANY of the following lands. `docs/api/` was added in 4.10.0 (`docs-api-reference` change); RG09 (`DocsApiCoverageTests`) catches *omission* of new methods, but **not staleness** of existing prose (e.g. exception tables, type relationships, wire-shape footnotes). PR-time review is the enforcement for staleness.
  - Change to public surface of `ISignalCliClient` / `ISignal*` / `ISignalEventService` interfaces → re-verify API-example signatures compile in README **and** in `docs/api/<category>.md`.
  - Change to `AddSignalCli*` extensions (new overload, removed overload, signature change) → re-verify DI setup snippets in README **and** in `docs/api/di-options.md`.
  - New top-level pattern shipped (event-kind, derived exception type, options field, new optional package like `SignalCli.NET.HealthChecks`) → consider "API capabilities" table addition + Quick-start / Extended-example mention + per-category `docs/api/` doc.
  - `<SignalCliPackageVersion>` bump → re-check README's version mentions, migration tables, "TestBaseline ≥ N" claims.
  - **Type or DTO `[Obsolete]` / nullability / signature change → `grep docs/ '<TypeName>'` AND `grep README.md '<TypeName>'` BEFORE writing tasks.md for the capability** — discovered in `api-coverage-audit-followup` 4.10.0 post-merge: Cap 4 (`json-payment-receipt-nullable`) і Cap 5 (`identity-changed-deprecation`) обидва шіпнули production-side type change але прогавили doc-sync — `docs/api/messaging.md` залишився з застарілою згадкою `IdentityChangedException` як "підтип для re-install'ів" + `Receipt: byte[]` (не nullable). RG09 не fail'ив (тип все ще згадується), але prose-семантика стала misleading. Fix landed у окремому follow-up PR #21 (`docs(messaging): sync ... to 4.10.0`); the lesson: при API-changing capability **tasks.md мусить мати explicit `grep docs/ <affected-type>` task** як останній крок перед release commit'ом.
