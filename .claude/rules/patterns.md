...
- **Privacy still wins.** Root CLAUDE.md § Critical rule #1 is the contract: PII (bodies, phones, attachment payloads) never appears in `[LoggerMessage]` templates at `Information+`. The `PrivacyLoggingTests` suite asserts on `EventId`, not text — so renaming a message won't accidentally break privacy verification.
...
- **Strong typing over magic strings:** `TextStyleMode` enum, not `string? mode = "styled"`. For protocol values that must compare case-insensitively, use `StringComparison.OrdinalIgnoreCase`; reserve `ToUpperInvariant()` for values crossing the process boundary (root CLAUDE.md § Critical rule #5).
...
- **Tag values are low-cardinality and PII-free.** The canonical set of tag keys is exactly `{method, status, trigger, event_type}`. `MeterTagValues_AreOnlyKnownEnumLiterals` in `ObservabilityPrivacyTests` pins this — if you add a new tag key, you MUST extend the test's `knownTagKeys` set and re-justify in the test fixture why the key is PII-free. Adding `account`/`phone`/`recipient`/`body` as a tag value is a privacy invariant violation (root CLAUDE.md § Critical rule #1 — observability extension); the test catches it via literal-substring asserts on seed PII.
